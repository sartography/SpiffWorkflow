class BpmnConverter:
    ...
    def mapping_to_dict(self, mapping, **kwargs):
        ...
        return {str(k): self.registry.convert(v, **kwargs) for k, v in mapping.items()}

    def mapping_from_dict(self, mapping, key_class=None, **kwargs):
        """Restore a mapping from a dictionary of strings -> objects."""
        if key_class is None:
            return {k: self.registry.restore(v, **kwargs) for k, v in mapping.items()}
        else:
            return {key_class(k): self.registry.restore(v, **kwargs) for k, v in mapping.items()}
